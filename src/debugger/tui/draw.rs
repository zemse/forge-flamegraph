...
use crate::debugger::op::OpcodeParam;
use alloy_primitives::U256;
use foundry_compilers::sourcemap::{Jump, SourceElement};
...
impl DebuggerContext<'_> {
    ...
    pub(crate) fn draw(
        &self,
        // terminal: &mut super::DebuggerTerminal,
        acc: &mut Vec<Acc>,
    ) -> io::Result<()> {
        // terminal.draw(|f| self.draw_layout(f, acc)).map(drop)
        self.draw_layout(acc);
        Ok(())
    }

    #[inline]
    // fn draw_layout(&self, f: &mut Frame<'_>, acc: &mut Vec<Acc>) {
    ...
    //     let size = f.size();
    //     let width_color = if size.width >= min_width {
    //         Color::Green
    //     } else {
    //         Color::Red
    //     };
    //     let height_color = if size.height >= min_height {
    //         Color::Green
    //     } else {
    //         Color::Red
    //     };
    ...
    /// |-----------------------------|
    /// ```
    // fn vertical_layout(&self, f: &mut Frame<'_>, acc: &mut Vec<Acc>) {
    //     let area = f.size();
    ...
    //         .constraints([
    //             Constraint::Ratio(100 - h_height, 100),
    //             Constraint::Ratio(h_height, 100),
    //         ])
    ...
    /// |-----------------|-----------|
    /// ```
    // fn horizontal_layout(&self, f: &mut Frame<'_>, acc: &mut Vec<Acc>) {
    fn horizontal_layout(&self, acc: &mut Vec<Acc>) {
        ...
        // let [app, footer] = Layout::new()
        //     .direction(Direction::Vertical)
        //     .constraints([Constraint::Ratio(100 - h_height, 100), Constraint::Ratio(h_height,
        // 100)])     .split(area)[..]
        // else {
        //     unreachable!()
        // };
    ...
    //     let lines = vec![
    //         Line::from(Span::styled(l1, dimmed)),
    //         Line::from(Span::styled(l2, dimmed)),
    //     ];
    //     let paragraph = Paragraph::new(lines)
    //         .alignment(Alignment::Center)
    ...
    // fn draw_src(&self, f: &mut Frame<'_>, area: Rect, acc: &mut Vec<Acc>) {
    fn draw_src(&self, acc: &mut Vec<Acc>) {
        let text_output = self.src_text(acc);
    ...
    fn src_text(&self, acc: &mut Vec<Acc>) {
        ...
        // let actual: Vec<_> =
        // source_code[actual_start..actual_end].split_inclusive('\n').collect();
        // let mut after: VecDeque<_> = source_code[actual_end..].split_inclusive('\n').collect();

        // if source_element.jump == Jump::In || source_element.jump == Jump::Out {

        // let last_acc = acc.last();
        let new_acc = Acc {
            source_element: source_element.clone(),
            source_code: source_code[actual_start..actual_end].to_string(),
            current_step: self.current_step().clone(),
        };
        // if last_acc.is_none() || &new_acc != last_acc.unwrap() {
        acc.push(new_acc);
        // }
    ...
    //         let mut min_start = cmp::max(
    //             self.current_step as i32 - height + extra_top_lines,
    //             abs_min_start,
    //         ) as usize;
    ...
    //             let bg_color = if is_current_step {
    //                 Color::DarkGray
    //             } else {
    //                 Color::Reset
    //             };
...
//         }
//         spans.push(Span::styled(
//             alloy_primitives::hex::encode([byte]),
//             f(i, byte),
//         ));
//     }
// }
